...
class MetadataExtractor:
    ...
    def __init__(self, output_dir: Union[str, Path]):
        ...
        # ⚡ Performance: Cache for style-related computations
    ...
    def _get_border_info(self, cell: openpyxl.cell.Cell) -> Dict[str, str]:
        ...
        if style_id in self._style_cache and "borders" in self._style_cache[style_id]:
        ...
        self._style_cache.setdefault(style_id, {})["borders"] = borders
        return borders
    ...
    def _get_cell_style_string(self, cell: openpyxl.cell.Cell) -> str:
        ...
        # ⚡ Performance: Check cache first
        style_id = cell.style_id
        if style_id in self._style_cache and "css" in self._style_cache[style_id]:
            return self._style_cache[style_id]["css"]
        ...
        res = "; ".join(styles)
        self._style_cache.setdefault(style_id, {})["css"] = res
        return res
    ...
    def _get_unit_info(self, cell: openpyxl.cell.Cell) -> Optional[str]:
        ...
        # ⚡ Performance: Check cache first
        style_id = cell.style_id
        if style_id in self._style_cache and "unit" in self._style_cache[style_id]:
            return self._style_cache[style_id]["unit"]

        fmt = cell.number_format
        if not fmt or fmt == "General":
            self._style_cache.setdefault(style_id, {})["unit"] = None
            return None
        ...
        self._style_cache.setdefault(style_id, {})["unit"] = res
        return res
    ...
    def _generate_metadata_and_html(
        ...
        for r_idx, row in enumerate(rows_gen, min_r):
            ...
            for c_idx, (cell, cell_f) in enumerate(zip(row, row_f if row_f else row), min_c):
                ...
                # ⚡ Performance: Pre-calculate values to avoid redundant computations
                val = cell.value
                clean_val = (
                    val.isoformat()
                    if isinstance(val, (date, datetime))
                ...
                cell_info = {
                    ...
                    "row": r_idx,
                    "col": c_idx,
                    "value": clean_val if cell.value is not None else None,
                ...
                td_html = self._cell_to_html_td(
                    cell, span, formula=formula, clean_val=clean_val, style_str=style_str, unit=unit
                )
    ...
    def extract(self, excel_path: Path, include_logic: bool = False) -> Dict[str, Any]:
        ...
        """
        # ⚡ Performance: Clear style cache for new workbook
