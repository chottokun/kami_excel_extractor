...
class MetadataExtractor:
    ...
    def _get_border_info(self, cell: openpyxl.cell.Cell, style_id: Optional[int] = None) -> Dict[str, str]:
        ...
        Args:
            ...
            style_id: Pre-calculated style_id (optional).
    ...
    def _generate_metadata_and_html(
        ...
        for r_idx, row in enumerate(rows_gen, min_r):
            ...
            current_row_html = []
            # ⚡ Performance: Use direct loop if ws_formula is not provided to reduce generator overhead
            if row_f:
            ...
            for c_idx, (cell, cell_f) in enumerate(cell_pairs, min_c):
                ...
                # ⚡ Performance: Pre-fetch style_id and coord once per cell
                style_id = getattr(cell, "style_id", None)
                coord = cell.coordinate
