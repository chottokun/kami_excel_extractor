...
class MetadataExtractor:
    ...
    def _parse_image_anchor(self, anchor: Any, sheet_name: str = "unknown") -> Tuple[Optional[int], Optional[int]]:
        """
        画像アンカーから行番号と列番号を抽出する。

        Args:
            anchor: 画像のアンカーオブジェクト。
            sheet_name: エラーログ用のシート名。

        Returns:
            Tuple[Optional[int], Optional[int]]: (行番号, 列番号) のタプル（1-indexed）。
        """
        row, col = None, None
    ...
    def _get_image_raw_data(self, img: Any, coord: str, sheet_name: str) -> Optional[Union[bytes, memoryview]]:
        """
        画像オブジェクトから生のバイナリデータを取得する。
        ...
        Returns:
            Optional[Union[bytes, memoryview]]: 取得したバイナリデータ。制限を超えた場合はNone。
        """
        raw_data = None
    ...
    def _process_single_image(self, img: Any, idx: int, sheet_name: str) -> Optional[Dict[str, Any]]:
        ...
        """
        row, col = self._parse_image_anchor(img.anchor, sheet_name)
    ...
    def _get_bounding_box(
        ...
        if hasattr(ws, "_images"):
            for img in ws._images:
                row, col = self._parse_image_anchor(img.anchor, "bounding_box_calculation")
