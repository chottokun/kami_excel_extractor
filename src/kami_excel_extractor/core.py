...
class KamiExcelExtractor:
    ...
    async def aextract_rag_chunks(self, excel_path: str, options: Optional[RagOptions] = None):
        ...
        extraction_options = ExtractionOptions(
            model=options.model,
            system_prompt=options.system_prompt,
            include_visual_summaries=True, # RAG context usually benefits from summaries
            use_visual_context=options.use_visual_context
        )
        structured_data = await self.aextract_structured_data(excel_path, options=extraction_options)
