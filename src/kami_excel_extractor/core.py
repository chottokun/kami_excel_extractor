...
class KamiExcelExtractor:
    ...
    async def _encode_image_to_base64_url(self, image_path: Path) -> str:
        ...
        # 2. 永続キャッシュ(SQLite)
        if use_cache:
            cached_url = await self.cache.aget_image_data_url(img_hash)
            if cached_url:
        ...
        async with lock:
            ...
            # キャッシュ保存
            if use_cache:
                await self.cache.aset_image_data_url(img_hash, result)
    ...
    async def aextract_structured_data(
        ...
        if use_cache:
            cached_raw = await self.cache.aget_raw_extraction(file_hash, self.opts.include_logic)
            if cached_raw:
                try:
        ...
        if raw_data is None:
            ...
            if use_cache:
                await self.cache.aset_raw_extraction(file_hash, self.opts.include_logic, json.dumps(raw_data))
