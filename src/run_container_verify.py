...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("container_full_verify.log"), logging.StreamHandler(sys.stdout)],
)
...
async def process_single_file(extractor, input_file, model, output_dir):
    ...
    try:
        ...
        options = ExtractionOptions(model=model, include_visual_summaries=True, use_visual_context=True)
