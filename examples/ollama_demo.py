...
async def run_ollama_demo():
    ...
    extractor = KamiExcelExtractor(base_url="http://localhost:11434", output_dir="data/output/ollama_demo")
    ...
    try:
        extract_options = ExtractionOptions(model=target_model, include_visual_summaries=False)
        results = await extractor.aextract_structured_data(excel_path, options=extract_options)
        ...
        chunks_map, structured_data = await extractor.aextract_rag_chunks(excel_path, options=rag_options)
