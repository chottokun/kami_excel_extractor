...
def test_cli_new_flags_passing(mock_extractor):
    ...
    test_args = ["kami-excel", "test.xlsx", "--include-logic", "--visual-summaries", "--dpi", "300"]
...
def test_cli_rag_new_flags_passing(mock_extractor):
    ...
    test_args = ["kami-excel", "test.xlsx", "--rag", "--include-logic", "--visual-summaries"]
    ...
    with (
        ...
        mock_inst.aextract_rag_chunks.assert_called_once_with(
            "test.xlsx",
            options=RagOptions(
                model=None, include_logic=True, include_visual_summaries=True, use_visual_context=True, dpi=150
            ),
        )
...
def test_cli_visual_summaries_explicit_false(mock_extractor):
    ...
    test_args = ["kami-excel", "test.xlsx", "--no-vision"]
