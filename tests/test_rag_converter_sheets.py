...
def test_convert_sheets_data_multiple():
    ...
    converter = JsonToMarkdownConverter()
    data = {"sheets": {"Sheet1": "Content1", "Sheet2": "Content2"}}
    result = converter._convert_sheets_data(data)
    expected_parts = ["# Sheet1", "Content1", "# Sheet2", "Content2"]
...
def test_convert_sheets_data_with_media():
    ...
    converter = JsonToMarkdownConverter()
    data = {"sheets": {"Sheet1": "Content1"}, "media": [{"filename": "image.png", "visual_summary": "Summary"}]}
    result = converter._convert_sheets_data(data)
    assert "# Sheet1" in result
...
def test_convert_sheets_data_nested_content():
    ...
    converter = JsonToMarkdownConverter()
    data = {"sheets": {"Sheet1": {"Table": [{"A": 1, "B": 2}, {"A": 3, "B": 4}]}}}
    result = converter._convert_sheets_data(data)
    assert "# Sheet1" in result
