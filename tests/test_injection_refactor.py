...
def test_inject_visual_data_to_html_baseline(output_dir):
    ...
    media_map = {
        "A1": [{"visual_data": "Insight for A1", "filename": "a1.png"}],
        "B2": [{"visual_data": "Insight 1 for B2"}, {"visual_data": "Insight 2 for B2"}],
        "C3": [{"other_field": "no visual data"}],
        "D4": [{"visual_data": "Insight for D4"}],
    }
    ...
    assert (
        "data-coord='B2' <div class='visual-insight'>[図表データ(B2)]: Insight 1 for B2</div>\n<div class='visual-insight'>[図表データ(B2)]: Insight 2 for B2</div>"
        in result
    )
...
def test_inject_visual_data_to_html_multiple_occurrences(output_dir):
    ...
    media_map = {"A1": [{"visual_data": "Insight"}]}
