...
def test_cell_to_html_td_styles(tmp_path):
    ...
    style_str_nf = extractor._get_cell_style_string(cell_no_fill)
    unit_nf = extractor._get_unit_info(cell_no_fill)
    html_no_fill = extractor._cell_to_html_td(cell_no_fill, None, style_str_nf, unit_nf)
    ...
    style_str_nc = extractor._get_cell_style_string(cell_no_color)
    unit_nc = extractor._get_unit_info(cell_no_color)
    html_no_color = extractor._cell_to_html_td(cell_no_color, None, style_str_nc, unit_nc)
...
def test_get_cell_style_string(tmp_path):
    ...
    cell = MagicMock()
    cell.fill.start_color.index = "FFFF0000"
    ...
    assert "background-color: #FF0000" in style_str
    assert "border-left: 1px solid black" in style_str
    assert "border-right: 2px solid black" in style_str
    assert "border-top: 3px solid black" in style_str
    assert "border-bottom: 1px dashed black" in style_str
