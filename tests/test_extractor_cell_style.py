from pathlib import Path
from unittest.mock import MagicMock

import pytest

from kami_excel_extractor.extractor import MetadataExtractor


def test_cell_to_html_td_styles(tmp_path):
    extractor = MetadataExtractor(output_dir=tmp_path)

    # Case 1: 8-digit color index (e.g., AARRGGBB) -> Should strip first 2 digits
    cell_8 = MagicMock()
    cell_8.coordinate = "A1"
    cell_8.fill.start_color.index = "FFFF0000"  # Red
    style_str = extractor._get_cell_style_string(cell_8)
    html_8 = extractor._cell_to_html_td(cell_8, None, "Test8", style_str, None)
    assert "background-color: #FF0000" in html_8
    assert ">Test8</td>" in html_8

    # Case 2: 6-digit color index (e.g., RRGGBB) -> Should keep as is
    cell_6 = MagicMock()
    cell_6.coordinate = "B1"
    cell_6.fill.start_color.index = "00FF00"  # Green
    style_str = extractor._get_cell_style_string(cell_6)
    html_6 = extractor._cell_to_html_td(cell_6, None, "Test6", style_str, None)
    assert "background-color: #00FF00" in html_6
    assert ">Test6</td>" in html_6

    # Case 3: '00000000' color index -> Should result in no background style
    cell_0 = MagicMock()
    cell_0.coordinate = "C1"
    cell_0.fill.start_color.index = "00000000"
    style_str = extractor._get_cell_style_string(cell_0)
    html_0 = extractor._cell_to_html_td(cell_0, None, "Test0", style_str, None)
    assert "background-color" not in html_0
    assert ">Test0</td>" in html_0

    # Case 4: No fill
    cell_no_fill = MagicMock()
    cell_no_fill.coordinate = "D1"
    cell_no_fill.fill = None
    style_str = extractor._get_cell_style_string(cell_no_fill)
    html_no_fill = extractor._cell_to_html_td(cell_no_fill, None, "TestNoFill", style_str, None)
    assert "background-color" not in html_no_fill
    assert ">TestNoFill</td>" in html_no_fill

    # Case 5: Fill without start_color
    cell_no_color = MagicMock()
    cell_no_color.coordinate = "E1"
    cell_no_color.fill.start_color = None
    style_str = extractor._get_cell_style_string(cell_no_color)
    html_no_color = extractor._cell_to_html_td(cell_no_color, None, "TestNoColor", style_str, None)
    assert "background-color" not in html_no_color
    assert ">TestNoColor</td>" in html_no_color

def test_get_cell_style_string(tmp_path):
    extractor = MetadataExtractor(output_dir=tmp_path)

    # Test bold and italic
    cell = MagicMock()
    cell.fill = None
    cell.border = None
    cell.font.b = True
    cell.font.i = True
    style_str = extractor._get_cell_style_string(cell)
    assert "font-weight: bold" in style_str
    assert "font-style: italic" in style_str
