...
async def test_extract_structured_data_basic(
    mock_litellm, mock_aio_open, mock_extract, mock_convert, extractor, sample_excel_path
):
...
@patch("litellm.acompletion", new_callable=AsyncMock)
@patch("kami_excel_extractor.core.aiofiles.open")
async def test_extract_structured_data_with_visual_summaries(
    mock_aio_open, mock_litellm, mock_extract, mock_convert, extractor, sample_excel_path, output_dir
):
    mock_convert.return_value = Path("dummy.png")
    ...
    mock_extract.return_value = {"sheets": {"Sheet1": {"media": [{"filename": media_filename, "coord": "A1"}]}}}
...
@patch("kami_excel_extractor.core.aiofiles.open")
@pytest.mark.asyncio
async def test_extraction_yaml_parsing_failure(
    mock_aio_open, mock_extract, mock_convert, mock_litellm, extractor, sample_excel_path
):
    mock_convert.return_value = Path("dummy.png")
