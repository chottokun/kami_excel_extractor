...
async def test_cache_media_missing_triggers_reextraction(tmp_path):
    ...
    cached_data = {"sheets": {"Sheet1": {"media": [{"filename": "missing.png"}]}}}

    with (
        patch.object(extractor.cache, "get_file_hash", return_value="hash123"),
...
async def test_cache_media_exists_uses_cache(tmp_path):
    ...
    cached_data = {"sheets": {"Sheet1": {"media": [{"filename": "exists.png"}], "html": "<table></table>"}}}
