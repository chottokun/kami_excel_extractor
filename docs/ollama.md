...
- **Docker コンテナからの接続**: Linux の Docker コンテナ内からホスト側の Ollama に接続する場合、`localhost` ではなく Docker ブリッジのゲートウェイ IP である `172.17.0.1` を使用してください。
    - 環境変数例: `LLM_BASE_URL=http://172.17.0.1:11434`
- **タイムアウト設定**: 方眼紙エクセルのような大規模なデータを Ollama (特に VLM) で処理する場合、推論に数分〜数十分かかることがあります。`.env` の `LLM_TIMEOUT` を `1800` (30分) 以上に設定することを強く推奨します。
