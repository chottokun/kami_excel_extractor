...
Difyでは、生成された **JSONLファイル** 、 **Markdownファイル群**、または本システムでRAG用に最適化した **ハイブリッド構造DOCXファイル** を「ナレッジ (Knowledge)」として取り込むことで、Excelのセマンティクスをそのまま検索に使用できます。

#### 連携手順 1: JSONLによる一括アップロード (推奨)

1. Difyの管理画面から **「ナレッジ」-> 「ナレッジを作成」-> 「テキストファイルからインポート」** を選択します。
2. 本システムで出力した `{stem}_rag.jsonl` をアップロードします。
3. アップロード設定画面にて、「**JSONL構造の解析**」を選択します。
4. `content` フィールドを **本文** に、`metadata` フィールドを **メタデータキー** としてマッピングします。

#### 連携手順 2: ハイブリッド構造DOCXによるインポート

Difyは標準の画像入りWordファイルのパース時、表内の画像が無視されたりテーブルレイアウトが著しく崩れたりする制約があります。本ツールの `--docx` オプションで生成したDOCXファイルはこの制約を完全に克服しています。

1. 本ツールで生成した `{stem}.docx` をDifyのナレッジ作成画面に直接アップロードします。
2. Difyのパーサーがテーブルテキストをパースし、最右列に埋め込まれた「**ビジュアルインサイト（画像詳細のテキスト記述）**」がテキストとして100%インデックス化されます。
3. 表外の画像とそのキャプションに含まれる `(表中座標: A1 / 画像座標: A3)` というテキスト座標キーによって、チャンク分割後でもテーブルの行データと画像（のキャプション）が完全に紐づいた状態で検索対象になります。
